...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  void _handleClick() async {
    ...
    } else if (selectedPaymentSetting!.type == PAYMENT_METHOD_VNPAY) {
      VnpayServiceNew vnpayServiceNew = await VnpayServiceNew(
        paymentSetting: selectedPaymentSetting!,
        totalAmount: widget.selectedPricingPlan.amount.validate().toDouble(),
        subId: widget.selectedPricingPlan.id,
        isSub: true,
        onComplete: (p0) {
          String message;
          String transactionStatus = p0['transaction_status']?.toString() ?? 'unknown'; 
           print("mess: $transactionStatus");
                switch (transactionStatus) {
                  case '00':
                    message = languages.paymentSuccess;
                    savePayment(
                        data: widget.selectedPricingPlan,
                    ...
                    break;
                  case '01':
                    message = languages.transactionNotCompleted;
                    showTemporaryDialog(context, message);
                    break;
                  case '02':
                    message = languages.paymentCancelled;
                    showTemporaryDialog(context, message);
                    break;
                  case '03':
                    ...
                    break;
                  default:
                    message = languages.transactionStatusUnknown;
                    showTemporaryDialog(context, message);
                }
    ...
    } else if (selectedPaymentSetting!.type == PAYMENT_METHOD_MOMO) {
